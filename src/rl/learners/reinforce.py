...
from src.rl.optimizers.wrapped_optimizer import WrappedOptimizer

from src.rl.utils.functions import (
  np_to_torch,
  zero_optim_grads,
  discount_cumsum,
  compute_advantages,
...
class REINFORCE:
    ...
    @staticmethod
    def _check_entropy_configuration(entropy_method, center_adv, stop_entropy_gradient, policy_ent_coeff) -> None:
    ...
    def _compute_loss(self, obs, actions, rewards, valids, baselines):
      ...
      Args:
        ...
        actions (torch.Tensor): Actions fed to the environment with shape :math:`(N, P, A*)`.
    ...
    def _compute_advantage(self, rewards, valids, baselines):
      ...
      Returns:
        torch.Tensor: Calculated advantage values given rewards and baselines with shape :math:`(N \dot [T], )`.
      """
      advantages = compute_advantages(self._discount, self._gae_lambda, self.max_episode_length, baselines, rewards)
