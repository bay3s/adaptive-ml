...
from src.rl.utils.functions import unflatten_tensors
...
def _build_hessian_vector_product(func: Callable, params: List[torch.Tensor], reg_coeff: float = 1e-5):
  ...
  As outlined in  Pearlmutter, Barak A. "Fast exact multiplication by the Hessian." Neural computation 6.1 (1994).

  Args:
    func (callable): A function that returns a torch.Tensor. Hessian of the return value will be computed.
  ...
  def _eval(vector: torch.Tensor):
    """
    The evaluation function.

    Args:
      vector (torch.Tensor): The vector to be multiplied with Hessian.
...
class ConjugateGradientOptimizer(Optimizer):
  ...
  accept_violation: bool = False
    ...
    The search direction is computed using a conjugate gradient algorithm, which gives x = A^{-1}g, where A is a
    second order approximation of the constraint and g is the gradient of the loss function.
  ...
  def step(self, f_loss, f_constraint):  # pylint: disable=arguments-differ
    ...
    # Compute step size
    step_size = np.sqrt(2.0 * self._max_constraint_value * (1. / (torch.dot(step_dir, f_Ax(step_dir)) + 1e-8)))
  ...
  def state(self):
    """
    The hyperparameters of the optimizer.
  ...
  def __setstate__(self, state):
    ...
    if 'hvp_reg_coeff' not in state['state']:
      warnings.warn('Resuming ConjugateGradientOptimizer with lost state. This is fixed in garage>=2020.02.0.')
  ...
  def _backtracking_line_search(self, params, descent_step, f_loss, f_constraint):
    ...
    if (
    (
    torch.isnan(loss) or torch.isnan(constraint_val) or loss >= loss_before
